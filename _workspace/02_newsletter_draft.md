NVDA가 역사를 썼다 — "수요가 폭발했다" 85조 원의 증명, 그런데 왜 주가는 제자리인가
머뉴 MoneyNews | 2026-05-21 (목) | 돈이 되는 모든 것에 대한 소식

🇰🇷 코스피 7,272 / 코스닥 1,129 / 원달러 1,507
🇺🇸 S&P 500 7,337~7,398 / 나스닥 +1.54%(전일) / DOW 50,000+ / WTI $101~104 / BTC $77,100

📌 오늘의 핵심 — "수요가 폭발했다"
1. NVIDIA가 어젯밤 Q1 FY2027 실적을 발표했다. 매출 $81.6B(+85% YoY), EPS $1.87(컨센 $1.77 상회). 젠슨 황은 "수요가 급등 중이다. 에이전틱 AI가 도래했다"고 선언했다. Q2 가이던스는 $89~93B — 시장 기대를 다시 한번 앞섰다. 그런데 주가는 제자리다. 어닝 전날 +1.54% 올랐던 나스닥은 오늘 숨을 고른다. [CNBC](https://www.cnbc.com/2026/05/20/nvidia-nvda-earnings-report-q1-2027.html)
2. 어젯밤 스페이스X(SpaceX)가 나스닥 IPO 신청서를 공개했다. 티커 $SPCX, 목표 조달액 $75B, 기업가치 최대 $2조 달러. 사우디아람코(2019년 $294억)를 뛰어넘는 역대 최대 IPO가 될 수도 있다. 일론 머스크는 소매 투자자 비중을 20%+ 이상으로 늘리겠다고 밝혔다. [Bloomberg](https://www.bloomberg.com/news/articles/2026-05-20/musk-s-spacex-files-publicly-for-nasdaq-ipo-under-symbol-spcx)
3. 월마트(WMT)가 오늘 아침 Q1 실적을 발표했다. EPS $0.61(컨센 $0.58 +5.7% 비트), 이커머스 +22%, 광고 사업 +50% 급성장. 고물가에도 소비자는 버텼다. [BusinessWire](https://www.businesswire.com/news/home/20260514910833/en/Walmart-To-Host-First-Quarter-Earnings-Conference-Call-May-21-2026)

💰 관심 종목 현황
$TSLA $412.90 (-1.43%) 목표가 ~$355 — FSD 리투아니아 진출(EU 2번째), 테슬라 어닝 D+61 (7/21 예상)
$GOOGL $388.00 (±) 목표가 $428 — Google I/O 이후 Gemini 3.5 모멘텀 지속, 빅테크 AI 경쟁 선두
$NVDA ~$224 (±0%) 목표가 $269 — $81.6B 어닝 비트 but "소문에 사고 뉴스에 팔기" 패턴 재현
$COIN $194.18 (-3.07%) 목표가 ~$245 — Hyperliquid USDC 딜로 EBITDA $80M 감소 우려
$NFLX $89.33 (-0.52%) 목표가 ~$110 — $25B 자사주매입 실행 중, 광고 티어 성장 확인
$OKLO $61.94 (-2.0%) 목표가 $91 — Wolfe Research "Peer Perform" 신규커버, Q1 어닝미스 소화 중
$OHI $48.32 (+0.1%) 목표가 ~$54 — 헬스케어 리츠 안정, 금리 고점 수혜 기대
$SPY $735~739 (±) 목표가 N/A — S&P 500 7,337~7,398 레인지, NVDA 어닝 관망
$QQQ ~$490 (±) 목표가 N/A — 나스닥 100, 반도체 어닝 모멘텀 vs 채권 금리 압박

👀 오늘 주목할 것
Fear & Greed 지수 60~61 (탐욕). NVDA가 역대 최고 실적을 냈지만 주가는 제자리다. "어닝 비트 = 주가 급등"이라는 공식이 깨지는 날이다. 이미 기대가 충분히 반영된 종목에서 "좋은 실적"은 종종 차익 실현의 신호탄이 된다. 진짜 주목할 것은 Q2 가이던스 $89~93B — 이 숫자가 하반기 AI 인프라 투자 사이클의 온도계다. 오늘 장에서 시장이 어떻게 반응하는지가 6월 방향을 결정한다.

🔥 오늘의 픽 — 핀테크/결제/크립토 (목요일 테마)

🌡️ 시장 온도
Fear & Greed 60~61 (탐욕). NVDA 어닝 비트에도 시장은 담담하다. 30년물 국채 5.1%+는 여전히 상단 압박을 가한다. 그러나 BTC는 $77K+ 레벨을 지키며 크립토 섹터에 온기가 돈다. "어닝 시즌은 끝났고, 이제 내러티브 시즌이 시작된다" — 오늘부터 스페이스X IPO 로드쇼가 시장의 이야기를 바꾼다.

🎯 오늘의 테마
목요일은 핀테크/결제/크립토 날이다. 공교롭게도 오늘은 역사적인 날이기도 하다. 어젯밤 스페이스X가 S-1을 공개하면서 "누가 소매 투자자에게 이 세기의 IPO를 팔아줄 것인가"라는 질문이 시장에 던져졌다. 동시에 COIN은 Hyperliquid의 USDC 딜로 $80M EBITDA 타격 우려에 -3%다. 핀테크가 크립토를 삼키는 세계 vs 크립토가 핀테크를 우회하는 세계 — 두 내러티브가 오늘 충돌한다.

💡 픽 1: "세기의 IPO 수혜자, 아직 30% 남아 있다"
$HOOD Robinhood Markets (시총 $67B, 핀테크/브로커리지)

어젯밤 스페이스X S-1이 공개됐다. $75B 조달, $2조 달러 기업가치, 소매 비중 20%+. 이 숫자들이 공개되는 순간 한 종목이 직격 수혜가 됐다 — Robinhood다. 일론 머스크는 "소매 플랫폼 제외 없다"고 직접 부인했고, Robinhood는 이미 NYSE에 $1B 규모의 RV1 프리-IPO 펀드를 상장시켜 놓은 상태다. IPO 본게임은 D+21(6월 11일)이지만, 로드쇼는 지금 이 순간부터 시작된다.

배경은 두 겹이다. 첫째, Robinhood Q1 2026은 어닝 비트였다 — 매출 $1.07B (+15% YoY), Gold 구독자 430만 명 역대 최고, Banking 자산 $2B (전 분기 대비 5배 성장), 은퇴자산 $30B 돌파. 플랫폼이 브로커리지를 넘어 뱅킹으로 확장 중이다. 둘째, 예측 시장(Prediction Markets) + 선물 + 인덱스 옵션까지 — 도박에서 금융 인프라로 전환하는 $HOOD의 변신은 현재 진행형이다.

리스크: E*TRADE(Morgan Stanley)가 소매 배분을 선점할 가능성이 있다. Robinhood의 SpaceX 할당 규모가 기대보다 적을 경우 차익 실현이 나올 수 있다. 그러나 RV1 펀드는 IPO 이전에도 소매 투자자가 SpaceX에 노출될 수 있는 유일한 합법적 경로다.

📊 스코어카드
기술적 2/3 | 수급 2/3 | 카탈리스트 2/2 | 펀더멘털 2/2 | 신선도 +1 | 종합 9/10
📅 D-Day: 2026-06-11 스페이스X SPCX IPO (로드쇼 현재 진행)
현재가: $75.90 | 목표가: $98.44 (+30% 업사이드) | RSI: 모멘텀 구간 | P/E: 성장주 밸류 | 공매도: 전환 중

---
💡 픽 2: "비트코인 채굴기에서 AI 전력소로 — 1.5조 짜리 전략 전환"
$MARA Marathon Digital Holdings (시총 ~$9B, 비트코인 마이닝/디지털 인프라)

오늘 BTC가 $77K+ 레벨에서 브레이크아웃을 시도하자 MARA가 +6% 뛰었다. 그런데 이건 단순한 BTC 연동 종목 이야기가 아니다. 지난 4월 30일, Marathon은 $1.5B(약 2조 원)를 투자해 오하이오주 Hannibal의 Long Ridge Energy & Power — 505MW 천연가스 발전소 + 1,600에이커 부지 — 를 인수했다. 목적은 하나: 2027년 상반기 내 200MW 데이터센터 가동. 비트코인 마이너가 AI/HPC 인프라 기업으로 피벗하는 가장 공격적인 베팅이다.

경제학이 매력적이다. Long Ridge는 연간 $144M 조정 EBITDA를 즉시 기여한다. 15달러/MWh 이하의 올인 운영 비용은 상업용 전력의 절반 이하다. BTC 마이닝 + AI 컴퓨팅이라는 이중 모드 수익은 BTC 가격 변동성에 대한 헤지 역할을 한다. 규제 승인은 Q3 2026에 완료 예상 — 매달 진전 공시가 나올 수 있다.

리스크: BTC 가격 하락 시 주가 직결 하락. Q1 2026 순손실 지속 (전략적 전환 비용). Long Ridge 딜 규제 승인 지연 가능성.

📊 스코어카드
기술적 2/3 | 수급 1/3 | 카탈리스트 2/2 | 펀더멘털 1/2 | 신선도 +1 | 종합 7/10
📅 D-Day: 2026-06 Long Ridge 규제 승인 + 6월 초 BTC 생산량 보고
현재가: ~$13.1 (+6% 오늘) | 목표가: ~$15 | RSI: 과매수 근접 | BTC 보유량: 818K BTC

⚠️ 투자 조언이 아닌 데이터 기반 분석입니다.

📈 시장 포인트 — "어닝은 끝났다, 이제 두 개의 역사가 장을 움직인다"

어젯밤 NVIDIA는 역대 최대 분기 실적을 냈다. $81.6B, YoY +85%, 데이터센터 $75.25B(+92%). 젠슨 황의 말 한마디 — "수요가 폭발했다. 에이전틱 AI가 도래했다" — 는 AI 사이클이 아직 중반임을 확인했다. Q2 가이던스 $89~93B는 시장 기대치를 다시 상회했다. 그런데 주가는 제자리다. 역사적으로 NVIDIA 어닝 다음날 평균 상승률은 +2.4%이지만 중앙값은 +0.1% — 결국 오늘도 그 패턴이 재현됐다. "좋은 게 충분히 이미 가격에 녹아있다"는 월가의 냉정한 판단이다. [Quiver Quantitative](https://www.quiverquant.com/news/NVIDIA+Corporation+%28NVDA%29+Releases+Q1+2027+Earnings%3A+Blockbuster+Revenue%2C+Profit+and+EPS+Beat)

오늘 아침 두 번째 역사가 열렸다. 스페이스X S-1 공개 — 티커 $SPCX, 기업가치 최대 $2조 달러, 목표 조달액 $75B. 사우디아람코(2019년 $294억)의 2.5배다. IPO는 6월 11-12일 목표. 일론 머스크는 소매 투자자 배분 비중 20%+ 목표를 밝혔다. 리테일 접근성이 역대급 IPO의 차별점 중 하나다. 골드만삭스가 리드 주관사 포지션을 노리고 있다. [Bloomberg](https://www.bloomberg.com/news/articles/2026-05-20/musk-s-spacex-files-publicly-for-nasdaq-ipo-under-symbol-spcx) · [Fortune](https://fortune.com/2026/05/20/spacex-finally-files-ipo-prospectus-reveals-revenue-is-up-but-losses-are-too/)

월마트(WMT)는 오늘 아침 소비자 건강을 확인시켜줬다. EPS $0.61(예상 $0.58 +5.7% 비트), 이커머스 +22%, 광고 사업 +50%. 고물가 환경에도 저가 옴니채널이 작동했다. 다만 매출 $163.98B는 $164.43B 추정치를 소폭 하회 — 완벽하진 않지만 튼튼했다. [24/7 Wall St.](https://247wallst.com/companies/wmt/earnings/2026/Q1)

---

💸 핀테크/크립토 전선 — "COIN이 흔들리는 사이, PayPal과 Stripe는 전장을 바꾼다"

**1. Hyperliquid 딜, COIN에 $80M 펀치** — Hyperliquid가 Circle의 USDC를 공식 기축자산(AQA)으로 채택하는 대신, USDC 예치금 이자 수익의 90%를 Hyperliquid 생태계로 가져가는 딜을 발표했다. 기존엔 이 수익이 Circle과 Coinbase로 흘렀다. Compass Point 추정: Coinbase + Circle 합산 EBITDA 최대 $80M 연간 감소 가능. COIN은 오늘 -3.07% 하락 중이다. 더 큰 문제는 이 딜이 선례가 되면 — Polymarket, Jupiter 등 다른 DeFi 프로토콜도 같은 조건을 요구할 수 있다. 크립토 생태계에서 중개자(COIN, Circle)의 지대 수익이 서서히 해체되는 신호로 읽힌다. [CoinDesk](https://www.coindesk.com/markets/2026/05/18/hyperliquid-s-usdc-deal-could-supercharge-hype-pressure-circle-coinbase-margins-analysts-say)

**2. PayPal PYUSD, 1.6% 스테이블코인 시장 점유 확보** — PayPal의 자체 스테이블코인 PYUSD가 2025년 대비 600% 성장해 $3.6B 시총을 돌파했다. 전체 스테이블코인 시장의 1.6% 점유율. PayPal CEO Alex Chriss는 블록체인 통합을 전사적으로 밀어붙이는 중이다. Stripe의 Tempo 레이어1 블록체인에는 Anthropic, OpenAI, Visa, Klarna 등 14개 파트너가 합류 — 스테이블코인 결제 인프라 경쟁이 가속화된다. [DL News](https://www.dlnews.com/articles/markets/paypal-and-stripe-amond-fintechs-muscling-into-crypto-in-2026/)

**3. Stripe-PayPal 인수설, 여전히 진행 중** — Bloomberg가 2월 보도한 Stripe의 PayPal 부분/전체 인수 협상은 아직 "초기 단계"에서 결론이 나지 않았다. 두 거인의 합산 결제 인프라는 글로벌 온라인 거래의 40%+를 커버할 것으로 추산된다. 공식 발표 없이도 시장은 이 내러티브를 흘려보내지 않는다. [TechCrunch](https://techcrunch.com/2026/02/24/stripe-is-reportedly-eyeing-deal-to-buy-some-or-all-of-paypal/)

---

🏢 기업 뉴스

**$TSLA — EU 프런티어 확장**
이야기: Tesla가 FSD(지도 지원 자율주행)를 리투아니아에서 공식 서비스 개시했다. EU 두 번째 국가다. 테슬라는 현재 네덜란드에 이어 발트해 3국으로 유럽 FSD 확장을 빠르게 진행 중이다. 주가는 -1.43% ($412.90) — 넓은 시장 약세에 동조 중.
배경: Andrej Karpathy가 Tesla를 떠나 Anthropic으로 이적했다는 소식도 들어왔다. 테슬라 AI 핵심 인재의 이탈이지만, FSD 기술 자체의 모멘텀은 멈추지 않았다. Q1 2026 어닝에서 자동차 총마진 개선과 서비스/FSD 매출 성장이 확인됐다.
전망: 다음 어닝은 7월 21일(예상). FSD 유럽 확장 속도와 로보택시 허가 진행 상황이 핵심 변수다. [TradingKey](https://www.tradingkey.com/news/market-movers/261916864-market-movers-tsla-20260520)

**$GOOGL — Google I/O 여운, Gemini 3.5로 AI 전선 재정비**
이야기: 5월 19-20일 Google I/O에서 Gemini 3.5 패밀리를 공개했다. Gemini 3.5 Pro는 다음 달 출시 예정. CEO 순다르 피차이는 "올해는 AI에 전사적으로 올인한다"고 선언했다. Android XR 스마트글라스 발표도 Apple Vision Pro 대항마로 주목받았다.
배경: Google은 Q1 2026에 $109.9B 매출(+22%), 클라우드 $20B을 기록한 강자다. NVDA의 어닝 비트가 AI 수요 사이클을 확인시켜줌으로써 GOOGL의 AI 클라우드 전략도 재평가받는다.
전망: 주가 $388, 애널리스트 평균 목표가 $428(+10%). Google I/O 이후 순풍. 단기 리스크는 30년물 국채 5.1%+의 성장주 밸류에이션 압박. [Benzinga](https://www.benzinga.com/quote/GOOGL/analyst-ratings)

**$NVDA — "Agentic AI 도래" 선언, but 주가는 '소문에 사고 뉴스에 팔기'**
이야기: Q1 FY2027 실적은 경이롭다. $81.6B (+85%), 데이터센터 $75.25B (+92%), 비-GAAP EPS $1.87 (컨센 +5.4% 상회). 배당 인상. Q2 가이던스 $89.1~92.8B.
배경: Jensen Huang은 "빅테크 하이퍼스케일러들이 Blackwell 300 아키텍처를 흡수하기 위해 돌진 중"이라고 했다. AI 에이전트 도입이 추론 컴퓨팅 수요의 폭발적 증가를 만들고 있다.
전망: 역대 실적이지만 주가는 제자리. 이미 +40%나 오른 NVDA에 "비트=랠리" 공식은 없다. 진짜 촉매는 Q2 실적(8월) 또는 Rubin 아키텍처 출하 확인이다. [Quiver Quantitative](https://www.quiverquant.com/news/NVIDIA+Corporation+%28NVDA%29+Releases+Q1+2027+Earnings%3A+Blockbuster+Revenue%2C+Profit+and+EPS+Beat)

**$COIN — Hyperliquid 쇼크, 스테이블코인 패러다임이 흔들린다**
이야기: -3.07% ($194.18). Hyperliquid의 USDC 딜로 연간 최대 $80M EBITDA 손실이 예상된다. USDC를 발행하는 Circle과 쿠스터디-서비스로 수익을 올리는 Coinbase 모두 직격탄.
배경: GENIUS Act 통과로 스테이블코인 규제 명확성이 왔지만, 탈중앙 DeFi의 역습이 중앙화 거래소의 수익 모델을 흔든다. Coinbase는 USDC 수탁 서비스로 연간 수억 달러를 벌어왔다.
전망: 단기 약세 요인이지만, COIN은 여전히 규제 명확성의 최대 수혜주. 30년물 5.1%+ 환경에서도 크립토 관련 플랫폼 프리미엄은 유지된다. 매크로 개선 시 반등 여력 충분. [CoinDesk](https://www.coindesk.com/markets/2026/05/18/hyperliquid-s-usdc-deal-could-supercharge-hype-pressure-circle-coinbase-margins-analysts-say)

**$NFLX — $25B 자사주매입 실행 중, 광고 비즈니스 궤도 진입**
이야기: $89.33 (-0.52%). 역대 최대 $25B 자사주매입 프로그램이 실행 중이다. 스트리밍 1위 지위를 최대한 활용한 주주환원 전략.
배경: 광고 티어 성장과 글로벌 구독자 침투율 확대가 매출 다각화의 핵심이다. 경쟁사 Disney+가 흑자 전환에 성공한 가운데 NFLX는 더 성숙한 단계에 있다.
전망: 자사주매입이 EPS를 지탱한다. 프리미엄 콘텐츠(NFL 계약) + 광고 성장의 복합 효과로 안정적인 현금흐름 기반.

**$OKLO — Wolfe "Peer Perform" 신규 커버, 신뢰 회복 과제**
이야기: ~$61.94 (-2%). Q1 어닝 미스 + Wolfe Research "Peer Perform" 신규 커버가 압박. 목표가 $91.21(현재 대비 +47%) — 중장기 전망은 여전히 강세.
배경: NRC Aurora PDC 승인은 기업의 기술적 신뢰성을 확인했다. 그러나 상업 운전까지 경로가 길고 현재 매출은 $0다. NVDA와 LANL 파트너십은 AI 데이터센터 전력 수요의 잠재적 고객.
전망: SMR 섹터는 장기 테마다. 단기엔 어닝 미스 소화 + 규제 이정표 확인이 선결 과제.

---

🧩 흥미로운 이야기

1. **스페이스X S-1의 진짜 숫자**: 2025년 매출 $18B+, 순손실 $4.9B. 수익의 절반 이상이 스타링크(Starlink $11B). 일론 머스크는 Class B 주식의 93.6% 보유 + 85.1% 의결권. IPO 자금의 상당 부분은 화성 탐사 vs AI 인프라에 쓰일 예정. 지구 1위 로켓 회사를 사는 것이 아니라 "만물 컴퍼니"를 사는 것이다. [Fortune](https://fortune.com/2026/05/20/spacex-ipo-filing-s1-total-addressable-market-make-life-multiplanetary/) · [Al Jazeera](https://www.aljazeera.com/economy/2026/5/20/elon-musks-spacex-unveils-filing-for-blockbuster-ipo)
2. **MicroStrategy(MSTR), BTC 매도 가능성 첫 시인**: Michael Saylor이 5월 5일 처음으로 "채권 만기(2027~2028, $4.1B)에 따라 BTC 매도를 고려할 수 있다"고 언급했다. 2020년 이후 "절대 팔지 않겠다"던 스탠스의 첫 균열. MSTR은 현재 818K+ BTC 보유 ($64B 상당). 이 신호가 BTC 시장에 어떤 의미인지 장기 보유자들이 주시 중이다. [CoinDesk](https://www.coindesk.com/markets/2026/05/11/strategy-buys-535-bitcoin-for-usd43-million-days-after-signaling-potential-btc-sales)

🔭 딥다이브 — 어닝 캘린더 & 수급 신호

📅 실적 캘린더 (향후 2주)
- 오늘(5/21): $WMT Q1 완료(비트 ✓), $DE Q2 발표(결과 집계 중)
- 5/27: $MRVL (Marvell) Q1 FY27 실적 — AI ASIC 반도체 수요 확인 이벤트
- 6/3: $AVGO (Broadcom) Q2 FY26 실적 — AI 커스텀 칩 $73B 수주잔고 점검
- 6/11: $SPCX 스페이스X IPO (목표) — 역대 최대 규모, 소매 배분 20%+
- 7/21: $TSLA Q2 어닝 — 로보택시 확장 + FSD 유럽 진척 확인

📊 애널리스트 컨센서스 하이라이트
- $NVDA: 57명 Buy, 1명 Sell, 평균 목표가 $269. Q2 가이던스 $89~93B가 컨센서스 $88B 상회 → 목표가 추가 상향 사이클 예상
- $GOOGL: 63명 Strong Buy, 평균 목표가 $428. Google I/O 이후 AI 실행력 재확인
- $HOOD: 27명 Buy, 평균 목표가 $98.44. SpaceX IPO 로드쇼 기간이 단기 모멘텀
- $MARA: Clear Street $12→$12+ 목표가, BTC 브레이크아웃 + Long Ridge 딜 HPC 피벗 주목

💹 옵션 & 수급 신호
- $NVDA: 어닝 발표 후 콜 옵션 IV 급락(어닝 프리미엄 소멸). 단기 방향성 불투명 → 다음 이벤트(8월 어닝)까지 드리프트 구간
- $HOOD: 52주 고점 갱신 근접, 콜 옵션 관심 증가. SpaceX IPO 로드쇼 기간 "이벤트 드리븐" 트레이더 집중
- $COIN: 풋 옵션 매수 증가 (Hyperliquid 쇼크 헤지). 단기 풋/콜 비율 상승 중
- $BTC/크립토: BTC $77K 레벨 돌파 시도 — 크립토 관련주(MARA, COIN, HOOD) 전반 상승 모멘텀

🏦 스마트머니 동향

**ARK Invest (최근 동향)**: 5월 중순 이후 ARK는 TSLA, AMD, CRCL(Circle), HOOD, SHOP, CRSP 전방위 매도를 지속했다. 단기 차익 실현 또는 포트폴리오 리밸런싱으로 해석된다. 반면 Cerebras(CBRS) 추가 매수는 계속됐다 — AI 인프라 테마는 유지.

**헤지펀드 & 슈퍼인베스터**: Atreides Management가 TSLA 지분을 14,760주 추가 매입했다. 기관 수급 방면에서 Tesla에 신규 진입하는 헤지펀드가 꾸준히 포착된다. North Dakota State Investment Board는 COIN 지분을 새로 취득 (5/20 공시) — 규제 명확성 이후 기관 크립토 진입 트렌드 지속.

**내부자 거래 주목**: Coinbase CFO가 10b5-1 플랜에 따라 9,750주 매도 (자동 매도 스케줄, 개인 의사결정 아님). COIN의 전반적 내부자 매수는 포착되지 않는 중.

**스마트머니 포지셔닝 요약**: AI 인프라(반도체, 데이터센터)와 스페이스X IPO 연관 핀테크(HOOD, RV1)에 기관 관심이 집중. 단기 매크로 불확실성(30년물 5.1%+, 연준 스탠스) 속 선별적 포지션 구축이 관찰된다.

🎙️ 테크 팟캐스트 — "Google I/O의 뒷이야기, 출산율 급락, 그리고 Spotify 로고가 왜 논란인가"

1. **TBPN** "Google I/O Recap, Fertility Decline, Spotify Logo Drama" (2026-05-20)
   John Coogan과 Jordi Hays의 TBPN은 어젯밤 Google I/O 결산을 라이브로 해부했다. Gemini 3.5 + Android XR 스마트글라스 + Googlebook 발표를 테크 인사이더 관점에서 짚었다. Sundar Pichai의 "올인 AI" 선언이 과연 구체적 실행인지 아니면 마케팅인지를 두고 두 호스트가 설전을 벌인다. 두 번째 코너는 예상치 못한 주제 — 출산율 급락이 테크 산업과 장기 노동시장에 미칠 함의를 논했다. 인구 절벽이 AI 자동화 수요를 가속한다는 주장과, 반대로 소비 시장 축소가 기술 기업의 성장 이야기를 약화시킨다는 반론이 충돌했다. 마지막은 Spotify 로고 리브랜딩 논란 — 미세한 초록색 색조 변화가 트위터(X)에서 밈이 됐고, 브랜드 아이덴티티 vs 트렌드 대응의 딜레마로 이어졌다. 오늘 NVDA 어닝 + WMT + DE + SpaceX를 다루는 오늘자 에피소드도 곧 공개 예정. [TBPN](https://www.tbpn.com/)

<!-- slack_trigger: 2026-05-21 -->