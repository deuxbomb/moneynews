메모리얼 데이 전야 — 8주 랠리의 숨고르기, 화요일이 진짜 시작이다

머뉴 MoneyNews 오후 브리핑 | 2026년 5월 24일 (일) | 돈이 되는 모든 것에 대한 소식

---

🇰🇷 이번 주 한국 장 마감 (금요일 5/22 기준)
코스피 7,847 (+0.41%) / 코스닥 1,161 (+4.99%) / 원달러 1,490원대

코스닥이 하루 만에 5% 폭등했다. 정부 주도 AI·반도체 전략펀드 조성 기대감이 불을 붙였다. 코스피는 소폭 상승으로 마감했지만 외국인과 기관이 동시에 순매수하며 7,800선을 안정적으로 지지했다. 8주 연속 오른 미국 S&P 500의 호기를 배경으로 한국 증시도 추세적 우상향을 이어가는 중이다. 월요일(5/25)은 한국 시장 정상 개장이지만 미국 메모리얼 데이 휴장 탓에 글로벌 수급은 엷어진다. 실질적인 방향성은 화요일 미장 재개장에서 결정된다.

---

🇺🇸 미국 프리마켓 (금요일 5/22 마감 + 일요일 선물 기준)
선물: S&P 7,473 / 나스닥 26,344 / 다우 50,580 / WTI $96.60 / BTC $76,857

S&P 500은 8주 연속 상승(2023년 12월 이후 최장)으로 역대 최고 구간에 있다. 그러나 이번 주는 달력이 시장을 시험한다 — 월요일은 메모리얼 데이 휴장, 화요일 재개장과 동시에 GDP·PCE·빅 어닝이 집중된다. 이란 협상 진전 신호에 WTI는 주간 -7% 급락했고, 케빈 워시(Kevin Warsh) 신임 연준 의장이 금요일 취임했다. 인플레이션 전사로 알려진 워시의 첫 발언 방향이 주중 금리 기대에 직결될 것이다.

---

📌 이번 주 캘린더 — 메모리얼 데이 이후 쏟아지는 이벤트

1. **화요일 5/26 재개장**: 소비자신뢰지수(5월), AutoZone·Zscaler 실적 발표. 이틀치 선물 에너지가 한 방에 터진다.
2. **수요일 5/27**: Marvell(MRVL) + Salesforce(CRM) + Synopsys 어닝. 'NVDA 이후 AI의 온도'를 Marvell이 측정한다. 신규 주택 판매 데이터도 발표.
3. **목요일 5/28**: Q1 GDP 2차 추정 + PCE 물가(연준이 보는 인플레이션). Dell + Costco + Best Buy 어닝 트리플 헤더.
4. **금요일 5/29**: SpaceX(SPCX) IPO 로드쇼 D-6. 6/4 로드쇼 시작, 6/11 가격 결정, 6/12 나스닥 상장 예정. 역대 최대 IPO($1.75T 밸류에이션)의 카운트다운.

---

💰 관심 종목 현황 (금요일 5/22 마감 기준)

$TSLA  $417.85  (일중 $431.08까지)  목표가 $450~508  FSD 벨기에 2,000km 완주, 유럽 2번째 승인 임박
$GOOGL  $379.38  목표가 ~$470  $5T 시총 레이스, AI 검색 통합 완성 직후
$NVDA  $214.28  목표가 $300 (BofA)  Q1 $81.6B + Q2 $91B 가이던스 — 어닝 소화 중
$COIN  $184.99  목표가 $240  CLARITY Act 상원 본회의 표결 대기
$NFLX  $88.60  목표가 ~$1,100  $25B 자사주매입 진행 중, 광고 티어 성장 가속
$OKLO  $65.09  목표가 $80 (BofA Buy)  NVDA+LANL 파트너십, 14GW 파이프라인
$OHI  $48.32  배당수익률 ~7%  헬스케어 리츠 안정, 금리 하락기 재부각
$SPY  ~$747  S&P 500 8연속 주간 상승  역대 최고 구간
$QQQ  ~$487  나스닥 26,344  AI 랠리 지속

---

👀 오늘 밤(일요일) 주목할 것

선물 시장은 메모리얼 데이 월요일에도 제한적으로 운영된다(美 중부시간 정오까지). 일요일 밤 선물 움직임이 화요일 개장의 방향성을 선점한다. 이란 협상 추가 진전 보도, 케빈 워시의 초기 발언 톤, BTC 주말 변동성이 세 가지 체크포인트다. $76,857 수준의 BTC는 아직 '공포 없는 관망' 구간.

---

🔥 오늘의 픽 — 산업재/인프라/방산 (일요일)

🌡️ 시장 온도
8주 연속 랠리 후 메모리얼 데이 징검다리 주간. 공포탐욕지수는 68(탐욕) 구간을 유지하며 과열 경계선 밑에 있다. 방산·인프라 섹터는 $851.9B 국방세출법 확정과 골든돔 수주 가속으로 펀더멘털 지지선이 단단하다. 단, 이번 주 GDP·PCE 데이터가 워시 연준의 매파 성향과 맞물리면 금리 민감주에 단기 충격이 올 수 있다.

🎯 오늘의 테마
일요일 테마는 산업재/인프라/방산. 이번 주 AM 브리핑이 $KRMN·$GE로 항공우주와 방위를 이미 소화했으니, PM은 **'지상 인프라'** 쪽으로 각도를 튼다. 미국 전력망과 통신망을 짓는 도급업자들 — AI 데이터센터 전력 수요가 폭발하면서 가장 바쁜 사람들이 바로 이 섹터다.

---

💡 픽 1: 전력망의 총도급사 — CEO가 직접 무대에 선다

$PWR — Quanta Services (시총 $1,080억, 인프라 도급)

**이야기**: 5월 22일, Quanta Services가 $10억 규모 자사주매입 프로그램을 조용히 발표했다. 같은 날 이사회는 조셉 킴을 신규 이사로 선임했다. 그리고 3일 후인 5월 27일, Duke Austin CEO와 Jayshree Desai CFO는 직접 KeyBanc Capital Markets Industrials Conference 무대에 오른다. CEO+CFO 동반 출격은 단순 IR이 아니다 — 보통 새 가이던스나 전략 업데이트를 동반한다. 이미 Q1에서 EPS $2.68로 컨센서스를 30.1% 상회한 회사가 이런 무대에 빈손으로 나오는 법은 없다.

**배경**: Quanta Services는 고압 송전선, 천연가스 파이프라인, 통신망 인프라를 시공하는 '픽앤샤블 인프라'의 실체다. AI 데이터센터 붐이 전력망 수요를 끌어올리면서 Quanta의 수주잔고는 Q1 기준 $48.5B 역대 최고를 기록했다. Q1 매출은 $7.9B으로 YoY +26%이었고, 연간 가이던스는 매출 $34.7~35.2B, 조정 EPS $13.55~14.25로 상향됐다. BofA는 "backlog strength"를 근거로 목표가를 추가 상향했다.

**전망/리스크**: 금리 압박이 인프라 자금 조달 비용을 높이는 것이 단기 리스크다. 그러나 전력망 투자는 정부 인프라법(IIJA) 및 국방·AI 예산과 연동되어 있어 수요의 구조적 하방이 제한된다. RSI 67로 단기 과열 구간이나, 추세 모멘텀 관점에서는 ATH 인근에서 새 고점을 노리는 설정이다.

📊 스코어카드
기술적 1/3 | 수급 1.5/3 ($1B 바이백+BofA 목표가 상향) | 카탈리스트 2/2 (KeyBanc 5/27 CEO+CFO 발표) | 펀더멘털 2/2 (매출 +26%, EPS +30% 비트) | 신선도 +1 (14일 내 첫 픽) | 종합 7.5/10
📅 D-Day: 2026-05-27 (KeyBanc Industrials Conference, Boston)

---

⚠️ 오늘 2번째 후보 없음 — 일요일 산업재/인프라 발굴 현황

스크리너 결과: #1(과매도) + #4(어닝서프라이즈) + #7(신저가+내부자)를 돌렸으나, 14일 이내 dated 카탈리스트를 갖춘 적격 후보가 $PWR 1개뿐이었다. $KTOS(Kratos)는 Q1 +22.6% 성장에도 불구, 198건의 내부자 매도(매수 0건)와 D+14 카탈리스트 부재로 탈락했다. $LDOS(Leidos)는 -30% YTD로 52주 저점 구간이지만 근기 카탈리스트가 확인되지 않았다. 이번 주 AM 픽 $KRMN·$GE에 이어 방산/인프라 섹터 발굴이 소진된 측면도 있다. 오늘은 $PWR 1개로 갈음한다.

⚠️ 투자 조언이 아닌 데이터 기반 분석.

---

8주 연속 상승의 이면 — 워시 취임, 이란 협상, 그리고 화요일 재개장

메모리얼 데이 3일 연휴 직전인 금요일, 미국 시장은 다우 50,580(역대 최고)으로 문을 닫았다. 그러나 표면의 랠리 아래엔 두 가지 복병이 잠들어 있다. 첫 번째는 케빈 워시다 — 폴 인플레이션 파이터로 알려진 그가 연준 의장으로 취임한 바로 그날, "물가를 잡는 것이 최우선"이라는 신호를 내보냈다. 시장은 지금까지 이를 쿨하게 소화했지만, 5/28 PCE 발표 이후 그의 첫 통화정책 논평이 나오면 분위기가 달라질 수 있다. 두 번째는 이란이다 — 협상 진전 기대에 WTI는 주간 -7% 폭락, $96대까지 내려왔다. 전쟁이 끝나면 에너지·방산 섹터의 프리미엄이 동시에 수축된다. 화요일 재개장 첫날의 선물 방향이 이번 주의 나침반이 될 것이다. [Yahoo Finance](https://finance.yahoo.com/markets/live/stock-market-today-sp-500-notches-longest-weekly-win-streak-since-2023-dow-climbs-to-record-high-223454966.html) · [Washington Post](https://www.washingtonpost.com/business/2026/05/22/kevin-warsh-sworn-fed-chair-facing-inflation-political-pressure/)

---

전력망·방산 인프라 — 예산이 집행된다

**$1,000억 + $48.5B: 인프라 도급 슈퍼사이클의 실증 데이터**
Quanta Services(PWR)가 Q1 수주잔고 $48.5B(역대 최고)와 함께 $1B 자사주매입을 발표했다. 이는 단순 재무 이벤트가 아니다 — 전력망 인프라 수요가 AI 데이터센터와 재생에너지 연결망 수요 급증으로 구조적으로 높아졌다는 경영진의 자신감 표현이다. 5월 27일 KeyBanc 컨퍼런스에서 CEO Duke Austin이 직접 설명할 예정. [PRNewswire](https://www.prnewswire.com/news-releases/quanta-services-announces-quarterly-cash-dividend-and-new-1-billion-stock-repurchase-program-302779832.html)

**골든돔 계약 흐름: Rocket Lab이 RTX 팀에 합류**
$151B 골든돔(SHIELD) 사업의 우주기반 요격체(SBI) 파트에서 Rocket Lab이 RTX 팀의 파트너로 공식 합류했다. RTX는 패트리어트·GEM-T·SPY-6 시스템으로 방어 레이어의 핵심을 맡는다. $3.2B Space Force 계약 수주사 12곳(SpaceX, Northrop, Lockheed, Anduril 포함)의 이행 속도가 이번 주 방산주의 관심 지점이다. [Motley Fool](https://www.fool.com/investing/2026/05/15/rocket-lab-targets-missile-defense-and-golden-dome/)

**FY2026 국방세출 $851.9B 확정 — 무기재고 2~5배 확대 집행 개시**
상원 26-3 통과한 FY2026 국방예산이 실제 계약 집행 단계에 진입했다. 통상 예산 서명 이후 60~90일 이내에 대형 계약 공시들이 쏟아진다. $KRMN(오늘 AM 픽, KBCM 컨퍼런스 D+3)·$GE(JAL 계약)를 포함해 방산 인프라 전반의 수주 뉴스가 6월에 집중될 가능성이 높다.

---

$TSLA — 유럽이 문을 연다

**FSD 벨기에 2,000km 완주 — 유럽 2번째 승인 가시권**
벨기에 플란데런 교통부 장관 Annick De Ridder가 5/22 확인했다: Tesla FSD(Supervised)가 일주일 만에 벨기에 도로 2,000km를 완주했다. 플란데런 행정부가 추가 제한 테스트 후 승인을 권고했으며, 벨기에 한 지역의 승인이 전국 자동 인정된다. 지난달 네덜란드(EU 최초 잠정승인)에 이어 벨기에까지 확보하면 EU 2번째 시장 개방이다. Tesla는 현재 11개국에서 FSD를 운영 중. [DriveTeslaCanada](https://driveteslacanada.ca/news/tesla-fsd-belgium-testing-update/) · [TechCrunch](https://techcrunch.com/2026/05/20/teslas-full-self-driving-software-is-creeping-into-europe/)

**$4.3B 배터리 + $2B AI 하드웨어 — Tesla 두 건의 조용한 공시**
미국 정부가 Tesla와 LG에너지솔루션 간의 $4.3B 배터리 공급 계약을 공식 확인했다. 동시에 Tesla의 최신 SEC 공시에서 $2B 규모의 AI 하드웨어 인수 내역이 포착됐다 — 어떤 회사인지 아직 비공개다. Semi 대량생산 개시와 맞물리면 배터리 공급 확보의 전략적 의미가 상당하다. $TSLA 주가는 일중 $431.08까지 터치 후 $417~424 구간에서 거래됐다.

**$OKLO — BofA Buy $80 재개, 14GW 파이프라인 유지**
NVIDIA·로스앨러모스국립연구소(LANL) 파트너십 이후 BofA는 Buy $80 커버리지를 유지하고 있다. Oklo는 여전히 무수익 단계지만 고객 파이프라인 14GW라는 숫자가 AI 전력 인프라 테마의 장기 내러티브를 지지한다. 2024년 IPO 이후 첫 상업 원자로 가동은 2028~2029년 목표.

---

SpaceX IPO D-19 — 역대 최대 공모의 카운트다운

S-1이 공개됐다. 목표는 명확하다 — 나스닥 SPCX, 6월 12일 상장, 공모 규모 ~$750억, 기업가치 $1.75T. 로드쇼는 6월 4~8일 사이 시작 예정이다. Elon Musk는 B주식 93.6% 의결권 통제를 유지하는 이중주식 구조를 설계했다. BlackRock이 지분 참여를 검토 중이라는 보도도 흘러나왔다. 한국 투자자 접근: HOOD(Robinhood)를 통한 소매 직접 청약 가능성, 미래에셋 $5B 참여, Tiger ETF 연계 등 루트가 논의 중이다. 머뉴 오늘 오전 DEEP에서 1,750조 역설을 깊게 다뤘다. [TradingKey](https://www.tradingkey.com/analysis/stocks/us-stocks/261904604-spacex-ipo-spcx-date-set-for-june-12-175-trillion-valuation-tradingkey) · [Motley Fool](https://www.fool.com/investing/2026/05/21/mark-your-calendar-the-spacex-ipo-key-dates/)

---

흥미로운 이야기 — 이번 주를 지배할 두 개의 숫자

1. **PCE 5/28**: 워시 연준의 첫 기준금리 시그널이 여기서 나온다. 4월 PCE가 3.5%를 넘으면 6월 금리동결 확률이 추가 상승한다. 금리 민감 섹터(리츠·유틸리티·헬스케어)에 직격 또는 안도 반응.

2. **MRVL 5/27 어닝**: AI 커스텀 칩(ASIC) 사이클의 실시간 체온계. 8일 전 AM 브리핑이 $MRVL 픽으로 집중 분석했지만, 수요일 발표 숫자가 반도체 섹터 전체의 다음 방향을 결정한다. 컨센서스: Q1 FY27 매출 $2.09B, EPS $0.63.

---

🔭 이번 주 딥 어닝 캘린더 — 화요일부터 목요일까지 쏟아진다

**Q1 GDP 2차 추정 + PCE (5/28 목요일)**
이번 주 최대 데이터 이벤트다. Q1 GDP 속보치(+3.2%)가 상향될지 하향될지, PCE 물가가 4월 기준 얼마나 나올지 — 워시 연준의 다음 발언 방향이 여기서 결정된다. 현재 금리 선물은 6월 동결, 9월 1회 인하를 70%로 반영 중.

**5/27 Marvell Technology (MRVL) — AI ASIC의 성적표**
컨센서스: 매출 $2.09B (+72% YoY), EPS $0.63. AI 커스텀 칩(Google TPU, Meta MTIA 파트너) 사이클의 실시간 온도계. 머뉴 5/18 AM에서 '픽'으로 분석한 종목. 가이던스가 $2.4B+ 나오면 반도체 섹터 전체 재점화 가능.

**5/27 Salesforce (CRM) — AI 에이전트 실적의 첫 본격 검증**
컨센서스: 매출 $10.0B, EPS $2.67. Agentforce(AI 에이전트)의 유료 전환율이 핵심 체크포인트다. 기업 소프트웨어 전반의 AI 프리미엄이 유지되느냐가 여기서 확인된다.

**5/28 Dell (DELL) + Costco (COST) + Best Buy (BBY)**
- Dell: AI 서버 수요 지속 여부 / $NVDA 포스트어닝 수혜 확인
- Costco: 소비 트레이드다운 vs. 멤버십 파워
- Best Buy: 소비 하방 리스크 최전선. '고소득 up, 저소득 down' 이중화 트렌드 검증

---

🏦 스마트머니 동향 (이번 주 주목 움직임)

**ARK Invest (5/22 금요일 마지막 거래일)**
지난주 ARK는 TSLA에 대해 기관 단위 대규모 순매수 전환 기조를 이어갔다. CBRS(Cerebras) 누적 포지션 확대도 지속. TSM·AMD는 여전히 매도 방향. ARK 포트폴리오의 WeeklyDelta는 "TSLA 재편입 + AI 퓨어플레이 집중" 패턴을 이어가고 있다.

**Bill Ackman / Pershing Square**
MSFT 신규 포지션 진입 확인(5/21). 빅테크 중 가장 늦게 담은 종목이 Microsoft — Azure AI 매출 성장 스토리가 밸류에이션 매력과 맞아떨어진 것으로 분석된다.

**SpaceX 소매 투자자 접근 루트**
Robinhood RV1 프리-IPO 펀드 NYSE 상장. Musk가 "소매 투자자 포함 확인" 발언 이후 HOOD 주가가 촉매로 작용 중. 6/11 가격결정 전까지 HOOD는 간접 플레이로 활용 가능.

**인사이더 (주목 종목)**
- $AMC: CEO 250K주 직접 매수 (5/23 확인) — 극단 저평가 인식 or 전략적 신호
- $ETN (Eaton): 이사 연속 매수 (5/8+5/11) 확인 — 전력망 데이터센터 내러티브 지속

---

🎙️ 테크 팟캐스트 — 이번 주 에피소드 현황

오늘(일요일)은 주요 팟캐스트 대부분이 주말 휴방이다. 지난 7일 로그 대조 결과:
- TBPN 5/20, All-In E274 5/18, Acquired Vanguard 5/18 — 모두 AM에서 이미 커버
- Lex Fridman / No Priors / 20VC: 오늘 기준 신규 에피소드 미확인

이번 주 예고: SpaceX IPO 로드쇼(6/4) 전후 All-In·TBPN·Lex Fridman에서 일론 머스크 IPO 특집 에피소드 가능성 높음. 다음 주 집중 커버 예정.

<!-- slack_trigger: 2026-05-24 PM -->